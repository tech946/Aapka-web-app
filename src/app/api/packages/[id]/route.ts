...
export async function GET(
  ...
  try {
    ...
    const { data: allPackages, error: fetchError } = await supabaseAdmin
      .from('packages')
      .select(
        'package_id, package_name, package_description, package_price, package_category_id, package_days, package_nights, travel_dates, booking_slots, booking_days, date_ranges, pickup_location, start_date, end_date, adult_price, child_price, infant_price, solo_traveller_enabled, solo_traveller_price, solo_traveller_only, solo_room_type, with_visa, adult_visa_price, child_visa_price, infant_visa_price, adult_discount_amount, child_discount_amount, infant_discount_amount, discount_start_date, discount_end_date, agent_discount, accept_payment, min_adults, surcharge_block_days_before, status, terms_html, inclusion_html, exclusion_html, overview, holiday_description_html, itinerary, thumbnail_image, gallery, pdf_url, created_at'
      )
      .eq('status', 'active');
