...
export async function POST(req: NextRequest) {
  try {
    ...
    const offers_b2b_rates = parseBool(form.get('offers_b2b_rates'));
    const commission_markup_structure = String(
      form.get('commission_markup_structure') || ''
    ).trim();
    const provides_transfers = parseBool(form.get('provides_transfers'));
    const fleet_details = String(form.get('fleet_details') || '').trim();
    const tripadvisor_google_rating_link = String(
      form.get('tripadvisor_google_rating_link') || ''
    ).trim();
    const existing_clients = String(form.get('existing_clients') || '').trim();
    ...
    const special_offers_for_us = String(
      form.get('special_offers_for_us') || ''
    ).trim();

    const sampleFile = form.get('sample_package_rate_card') as File | null;
    ...
    if (!commission_markup_structure)
      return NextResponse.json(
        { error: 'Commission / markup structure is required' },
        { status: 400 }
      );
    if (!fleet_details.trim())
      return NextResponse.json(
        { error: 'Fleet details is required (enter N/A if not applicable)' },
        { status: 400 }
      );
    if (!license_type)
    ...
    if (!special_offers_for_us.trim())
      return NextResponse.json(
        { error: 'Special offers for us is required' },
        { status: 400 }
      );

    let sample_package_rate_card_url: string | null = null;
    let company_registration_proof_url: string | null = null;

    try {
      if (sampleFile && typeof sampleFile === 'object' && sampleFile.size > 0) {
        sample_package_rate_card_url = await uploadB2BFile(
          sampleFile,
          'sample-rate-cards'
        );
      }
    ...
    const payload = {
      ...
      key_products,
      offers_b2b_rates,
      sample_package_rate_card_url,
      commission_markup_structure,
      provides_transfers,
      fleet_details,
      tripadvisor_google_rating_link: tripadvisor_google_rating_link || null,
      existing_clients: existing_clients || null,
