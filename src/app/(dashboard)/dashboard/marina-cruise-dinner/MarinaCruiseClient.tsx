...
import EditMarinaCruiseClient from './EditMarinaCruiseClient';
...
export default function MarinaCruiseClient() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('marina-cruise-dinners:changed', onChanged);
    return () => window.removeEventListener('marina-cruise-dinners:changed', onChanged);
  }, []);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await fetch(`/api/marina-cruise-dinners?${params.toString()}`, {
          method: 'GET',
  ...
  const handleConfirmStatusChange = async () => {
    ...
    try {
      const response = await fetch('/api/marina-cruise-dinners', {
        method: 'PUT',
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/marina-cruise-dinners/${packageId}`, {
        method: 'DELETE',
      });
            ...
            {!loading &&
              rows.map(p => (
                <tr key={p.package_id}>
                  ...
                  <td>{p.category ?? '-'}</td>
                  <td>{p.timing ?? '-'}</td>
                  <td>{p.adult_price ?? p.package_price ?? '-'}</td>
                  <td>{p.child_price ?? '-'}</td>
                  <td>
                    <div
