...
type MarinaExcludedDatesSelectorProps = {
  dates: string[];
  onDatesChange: (dates: string[]) => void;
};

export default function MarinaExcludedDatesSelector({
  dates,
  onDatesChange,
}: MarinaExcludedDatesSelectorProps) {
  ...
  return (
    <div className='form_row full_width'>
      ...
      <p className='tour-booking-days-hint'>
        Add specific dates that should never be bookable — overrides any booking
        days or specific available dates selected above.
      </p>
      ...
      {dates.length > 0 && (
        <div className='date_chips'>
          {dates.map(dateStr => {
            ...
            const display = parsed ? parsed.toLocaleDateString() : dateStr;
            return (
              <span key={dateStr} className='date_chip date_chip_excluded'>
                {display}
